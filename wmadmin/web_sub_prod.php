...
$heading = (mysql_escape_string($_REQUEST['subproduct']) != '')?mysql_escape_string($_REQUEST['subproduct']):'';
$description = (mysql_escape_string($_REQUEST['cat_id']) != '')?mysql_escape_string($_REQUEST['cat_id']):'';
$prod = (mysql_escape_string($_REQUEST['prod_id']) != '')?mysql_escape_string($_REQUEST['prod_id']):'';
...
if(count($catQueryResult)>0)
		...
		{*/
			...
			$categoryQuery = "insert into subproduct(subproduct,cat_id,prod_id) values('$heading', '$description','$prod')";
			$in=ExecuteQuery($categoryQuery);
			header("Location: add_sub_product.php?q=add");
	//}
