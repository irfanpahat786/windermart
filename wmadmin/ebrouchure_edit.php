...
$ebrochureid = (isset($_GET['ebrochureid']) and $_GET['ebrochureid'] != '' ) ?  $_GET['ebrochureid'] :  header("Location: ebrouchere_view.php?q=err");



 if ($_REQUEST['q']=='delfile')
			   {
				
				$memberQuery = "update ebrouchure set filepdf='' where ebrochureid='$ebrochureid'";
				...
				header("Location: ebrouchure_edit.php?ebrochureid=$ebrochureid");
			   }
...
$userQuery = "select * from ebrouchure where ebrochureid='$ebrochureid'";
$userQueryResult = $connectionObject->executeQuery($userQuery);
   if(!$userQueryResult)
	...
	$userRow = mysql_fetch_assoc($userQueryResult);
			$ebrochureid = $userRow['ebrochureid']; 			
...
<body>
<br>
<form name="memberform" action="ebrouchure_edit_inter.php" enctype="multipart/form-data" method="post" onSubmit="return validation();">
...
<tbody>
          ...
          <tr>
             <td  bgcolor="#FBFBFB" style="padding:20px;">User Status </td>
             <td  bgcolor="#FBFBFB">
			 <input type="hidden" name="ebrochureid" value="<?php echo stripslashes($userRow['ebrochureid'])?>" />
         ...
         <!--   <tr>
             <td  bgcolor="#FBFBFB">Projects*</td>
             <td  bgcolor="#FBFBFB"> <select name="projectsid" class="style77">
			  	<option value="">Select Projects</option>
			  <?php
			  		$projectsid = $userRow['projectsid'];
				    $catQry = "select projectsid, heading from projects order by projectsid asc";
					...
					{
			  ?>	
			  		<option value="<?php echo $catRow['projectsid']; ?>" <?php if($catRow['projectsid']==$projectsid) echo "selected"; ?>><?php echo $catRow['heading']; ?></option>
			  <?php
			  		}
			  ?>		  			  	 
			  </select></td>
         ...
         <tr>
          ...
          <!--<tr>
            ...
            <td rowspan="3" align="center" valign="top" class="style77">
			<?php 
			 if ($userRow['imgName']!='')
			  {
			?>
			<img src="img/img_not_available2.jpg" width="120" height="150" border="0"/><br />
<br />
<!--<a href="brouchure_edit.php?q=delimg&ebrochureid=<?=$ebrochureid?>">Delete</a>-->
			<!--<?php
			  }else
			  {
			?>
         ...
         <!-- <tr>
           ...
           <tr>
             <td  bgcolor="#FBFBFB">Upload File 
               </td>
             <td  bgcolor="#FBFBFB"><span class="style777">
              <input name="filepdf" type="file" id="filepdf" size="35" /><br />
[File Type:DOC, PDF, Size:Up to 8 MB]
			...
			<?php 
			 if ($userRow['filepdf']!='')
			  {
			?>
			<a href="../docs/ebrouchure_file/<?php echo stripslashes($userRow['filepdf'])?>" target="_blank">Download File</a> <br />
<br />
<a href="ebrouchure_edit.php?q=delfile&ebrochureid=<?=$ebrochureid?>">Delete</a>
			<?php
			  }else
			  {
			?>
