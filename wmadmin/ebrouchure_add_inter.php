...
$projectsid = (mysql_escape_string($_REQUEST['projectsid']) != '')?mysql_escape_string($_REQUEST['projectsid']):'';
...
{
	//File Size
	$filesize1=$_FILES["filepdf"]["size"]/(6024*6024);
	if($filesize1 > 8)
	{
		error_message ("You can upload maximum of 8 MB file! ");
	}
	//File Type
...
echo $detail = (mysql_escape_string($_POST['newsContent']) != '') ? mysql_escape_string($_POST['newsContent']):'';
		 ...
		 $adminQuery = "insert into  ebrouchure set projectsid = '$projectsid', heading = '$heading',Key_word = '$Key_word',Description = '$Description', numbering = '$numbering', priority = '$priority', status = 'active', filepdf='$fileName',imgName='$imgName', detail ='$detail',Add_title='$Add_title', createdate='".date("Y-m-d")."'	";
...
header("Location:  ebrouchere_view.php?q=add");
